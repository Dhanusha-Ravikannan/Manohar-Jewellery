...
const WeightFormPopup = ({
  ...
  const [showBarcode, setShowBarcode] = useState(false);
  const [selectedProductNo, setSelectedProductNo] = useState(null);
  const [product_number,setProductNumber]=useState(transform_text((productInfo.product_number) ?productInfo.product_number:"" ));
  const [finalWeight, setFinalWeight] = useState(productInfo.final_weight || "");
  const barcodeRef = useRef(null);
...
{showBarcode && selectedProductNo &&(
  <div
    ...
    <div>  <img src={imagess} alt="jewelery" style={{height:"80px",width:'80px'}} /></div>
</div> 
  </div>
)}
          </div>
